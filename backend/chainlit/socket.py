...
@socket.on("remove_datalayer")
async def remove_datalayer(sid, payload):
    """Handle a remove_datalyer sent by the User."""
    session = WebsocketSession.require(sid)
    context = init_ws_context(session)
    await context.emitter.emit("remove_element", payload)
